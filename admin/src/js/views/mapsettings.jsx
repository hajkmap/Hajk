...
$.fn.editable = function (component) {
  function edit (node, e) {
    ...
    function saveLayer () {
      ...
      if (visible) { node.parent().addClass('visible'); } else { node.parent().removeClass('visible'); }
      reset();
    }
...
class Menu extends Component {
      ...
      return (
        <div className='col-sm-12'>
          <label htmlFor='authGroups'>Tillträde &nbsp;
            <i className='fa fa-question-circle' data-toggle='tooltip' title='Ange AD-grupper separerade med kommatecken' />
          </label>
          <input id='authGroups' name='authGroups' type='text' onChange={(e) => { this.handleAuthGrpsChange(e); }} value={this.state.visibleForGroups} />
          <i className='fa fa-bars' data-toggle='tooltip' title='Visa tillgängliga AD-grupper' onClick={() => { this.toggleHidden(); }} />
        </div>
      );
    } else {
      return null;
  ...
  renderArticleContent () {
    if (this.state.mapOptions) {
      return (
        <MapOptions parent={this} model={this.props.model} />
      );
    }
    if (this.state.toolOptions) {
      return (
        <ToolOptions parent={this} model={this.props.model} />
      );
    }
