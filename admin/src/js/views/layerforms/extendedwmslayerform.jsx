...
class ExtendedWMSLayerForm extends Component {
  ...
  setVersion() {
    this.setState({ version: this.state.capabilities.version });
  }
  ...
  setLayerSettings(layer) {
    ...
    let currentLayer = this.state.capabilities.Capability.Layer.Layer.find(
      l => {
        return l.Name === layer.name;
      }
    );
    // om lagret har styles, hämta dessa, annars, returnera meddelande
    let layerStyles = currentLayer.Style ? (
      currentLayer.Style.map((style, index) => {
