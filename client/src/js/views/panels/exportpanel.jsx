...
var ExportPdfSettings = React.createClass({
    //resolutions = this.props.model.get('resolutions'),
    //paperFormats = this.props.model.get('paperFormats'),
    ...
    var width = pageSize(this.getFormat()).width,
      height = pageSize(this.getFormat()).height;
        ...
        if (this.props.model.get("layout") == 2) {
            ...
            // changed here too.
            return {
                width: ((width / 25.4)) * (1 - 0.10 * 2),
                height: ((height / 25.4)) * (1 - 0.10 * 2)
            }
        } else if(this.props.model.get("layout") == 3) {
            return {
                width: ((width / 25.4)) * (1 - 0.10 * 2),
                height: ((height / 25.4)) * (1 - 0.12 * 2)
            }
        } else {
            return {
                width: ((width / 25.4)),
  ...
  render: function () {
    ...
    resolutionOptions = this.props.model.get('resolutions').map((s, i) => {
...
paperFormatOptions = this.props.model.get('paperFormats').map((s, i) => {
