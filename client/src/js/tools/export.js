...
module.exports = ToolModel.extend({
    ...
    generateScaleBar: function() {
        ...
        clone.css({
            "width": $('.ol-scale-line-inner').width() + 2,
            "border-radius": "0px",
            "padding": "4px",
            "background": "white"
        });
     ...
     *       Call the toDataUrl to produca a Base64-encoded image url.
     *       Create a download anchor and trigger click to prevent popup.
     * @param {function} callback
     */
    exportMap: function(callback) {
        ...
        map.once('postcompose', (event) => {
            ...
            ,   exportImage
            ;
            canvas = this.cloneCanvas(event.context.canvas);
            ...
            img.onload = function() {
                context.drawImage(img, 10, canvas.height - 25);
    ...
    exportPDF: function(callback) {
        this.exportMap((href) => {
            ...
            $.ajax({
                ...
                type: 'post',
                contentType: 'text/plain',
                data: 'pdf;' + encodeURIComponent(href),
                success: response => {
                    var anchor = $('<a>Hämta</a>').attr({
                        href: response,
                        target: '_blank',
