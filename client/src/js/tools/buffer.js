...
var BufferModel = {
  ...
  configure: function(shell) {
    ...
    if (typeof this.get("geoserverNameToCategoryName") === "string") {
      try {
        this.set(
          "geoserverNameToCategoryName",
          JSON.parse(this.get("geoserverNameToCategoryName"))
        );
      } catch (e) {
  ...
  activateBufferMarker: function() {
    if (this.get("bufferMarkKey") == undefined) {
      this.set(
        "bufferMarkKey",
        this.get("olMap").on("singleclick", this.placeMarker.bind(this))
      );
    }
  },
  ...
  buffer: function() {
    if (!this.get("varbergVer")) {
      ...
      if (buffered) {
        this.get("bufferLayer")
          .getSource()
          .addFeatures(buffered);
        return true;
      } else {
        return false;
      }
      ...
      for (var i = 0; i < this.get("layersCollection").length; i++) {
        if (
          this.get("layersCollection").models[i].getVisible() &&
          this.get("notFeatureLayers").indexOf(
            this.get("layersCollection").models[i].id
          ) != -1
        ) {
  ...
  putFeaturesInResult: function(res) {
    ...
    for (var i = 0; i < featureMembers.length; i++) {
      ...
      var coordinate = featureMembers[i].getElementsByTagName(
        "gml:coordinates"
      )[0].innerHTML;
    ...
    var categoryPrefix =
      '<div class="panel panel-default layer-item"><div class="panel-heading unselectable"><label class="layer-item-header-text">';
    var endCategoryToStartLayers =
      '</label></div><div class="panel-body"><div class="legend"><div>';
  ...
  popupIcons: function(event) {
    ...
    var point = new ol.geom.Point([coord[1], coord[0]]);
    // clear layer
    this.get("layer_popup")
      .getSource()
      .clear();
