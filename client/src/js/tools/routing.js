...
var RoutingModel = {
  ...
  plotRoute: function(res, map, layer, layer_drawing) {
    ...
    var routePath = new ol.format.Polyline({}).readGeometry(
      res.routes[0].overview_polyline.points
    );

    routePath = new ol.format.Polyline({}).readGeometry(
      res.routes[0].overview_polyline.points,
      {
    ...
    var centerLat =
      (this.get("position").latitude + this.get("position").latitudeEnd) / 2;
    var centerLon =
      (this.get("position").longitude + this.get("position").longitudeEnd) / 2;
    map
      .getView()
      .setCenter(
        ol.proj.transform(
  ...
  drawRoute: function(steps) {
    var routePath = new ol.format.Polyline({}).readGeometry(steps);
