...
function arraySort(options) {
    ...
    function comparer(a, b) {
        var a_s = str(a.getProperties()[options.index]) // Strängjämförare.
        ,   b_s = str(b.getProperties()[options.index]) // Strängutmanare.
        ...
        if (a.getProperties()[options.index] === b.getProperties()[options.index]) return 0;

        if (a_s > b_s) return  1;
        if (a_s < b_s) return -1;

        // Strängdelen är samma, jämför nummer.
        a_n = num(a.getProperties()[options.index]);
        b_n = num(b.getProperties()[options.index]);
        ...
        // jämför suffix.
        ans = strnum(a.getProperties()[options.index]);
        bns = strnum(b.getProperties()[options.index]);

        if (ans > bns) return 1;
        if (ans < bns) return -1;

        // Övriga matchningar sorteras alfabetiskt.
        return a.getProperties()[options.index] > b.getProperties()[options.index] ? 1 : -1;
...
module.exports = ToolModel.extend({
  ...
  getLayers: function () {
    var filter = (layer) => {
      ...
      console.log(criteria, layer.get('id'))
