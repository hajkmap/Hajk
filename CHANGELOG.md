...
- Admin: Map settings — enable/disable layers on a map via drag-and-drop; `PUT /api/v3/maps/:mapName/layers` replaces direct `LayerInstance` placements. Removing a layer from the map drop zone removes its link; `visibleAtStart` controls start visibility without unlinking. Unified settings tabs across admin settings pages.
- Admin: map content — atomic save via `PUT /api/v3/maps/:mapName/content` (layers + group placements in one transaction); `GET /api/v3/maps/:mapName/content` for combined load. Separate `PUT /layers` and `PUT /groups` remain for targeted updates.
- Admin: Map Content — separate Placement and Draw order sub-tabs; catalog-based draw order with positional drag-and-drop (insert above/between/below), status chips, and remove from draw order.
- Admin: Map draw order (K11) — ritordning persisted as `LayerInstance.zIndex` via `PUT /maps/:mapName/layers` (order-only) or `PUT /maps/:mapName/content` (with placement); admin invalidates `layersByMap` and `mapContent` on save. Client `layersConfig` exposes `drawOrder` sorted ascending (higher = on top).
