...
- Client + Admin + Backend: AttributeEditor - a brand-new editing tool for WFS-T layers with table and form editing modes: bulk attribute editing, geometry drawing and editing through the Sketch plugin, duplicate/split/merge of features, per-column faceted filtering, Excel export and mobile support. Editing services are configured in a new Admin tool view; the backend serves WFST layer metadata via new `/api/v2/ogc/wfst` endpoints (with an optional `WFS_ALLOWED_HOSTS` allowlist), while the WFS/WFS-T traffic itself goes directly from the browser to the map server, so existing per-user authorization (e.g. Kerberos/NTLM) applies. [#1675](https://github.com/hajkmap/Hajk/issues/1675)
...
- Client + Admin: WMTS - Added a third request encoding, "KVP (URL-mall)". It generates a KVP `GetTile` query string where the per-tile parameters are left as `{TileMatrix}`/`{TileRow}`/`{TileCol}` placeholders, built from the `GetTile` endpoint advertised in `OperationsMetadata`. Some servers (e.g. GeoServer/GWC) only work with this form.
- Admin: WMTS - The layer form now uses much more of the GetCapabilities response. Styles are picked from a dropdown (with the server's default preselected) instead of being typed by hand, the legend URL is taken from `LegendURL`, the layer's `ows:Title`/`ows:Abstract` and the service's provider info prefill the caption, info text and attribution, advertised `Dimension` values (such as GeoServer's `FORMAT_OPTIONS`) get a field of their own, and `sizes` is now derived from the layer's own `TileMatrixSetLimits` rather than the full matrix set, so the client stops requesting tiles outside the layer's data extent.
...
- Client: WMTS - `requestEncoding` was never copied from the layer config into the OpenLayers source, so every WMTS layer was rendered using REST encoding no matter what was configured, and layers saved as KVP requested the bare service URL for every tile. The value is now passed through. Note that layers already saved with KVP will change behaviour — from silently REST to actually KVP. Layers with no `requestEncoding` at all keep working exactly as before, since REST is used as the default.
- Client: WMTS - Dimensions are now passed to the OpenLayers source. Without them, a `{FORMAT_OPTIONS}` placeholder in a server's own `ResourceURL` template (standard in GeoServer/GWC) was substituted with the literal string `undefined`, breaking every tile request.
- Client: WMTS - A layer's own `crossOrigin` setting from the admin UI is now respected; it was previously always overridden by the map-level value.
- Client + Admin: WMTS - High-DPI variants are now actually rendered at high resolution. The correct tiles were requested (e.g. GeoServer/GWC's `EPSG:3008x2` gridset with `FORMAT_OPTIONS=dpi:180`), but OpenLayers' `tilePixelRatio` was never set, so the larger tiles were simply scaled back up and looked exactly as soft as standard ones. The tile grid is now expressed in CSS pixels with the extra density declared separately, which requires no configuration change — the pixel density is derived from the variant's tile size relative to the standard matrix set's. Admin shows the resulting density per configured level. Note that if the high-DPI matrix set has fewer levels than the standard one, the innermost zoom level still renders at standard sharpness.
