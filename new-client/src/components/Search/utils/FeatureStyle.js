...
export default class FeatureStyle {
  ...
  constructor(options) {
    ...
    this.#defaultSelectedStyleSettings = this.#getDefaultSelectedStyleSettings();
    this.#defaultHighlightStyleSettings = this.#getDefaultHighlightStyleSettings();
  }

  #getDefaultSelectedStyleSettings = () => {
    const strokeColor =
    ...
    const textStrokeColor =
      this.#options.selectionTextStroke ?? "rgba(0, 255, 0, 0.5)";
  ...
  getFeatureStyle = (feature, featureTitle, displayFields, type) => {
    ...
    const settings =
      !type || type === "selection"
        ? this.#defaultSelectedStyleSettings
