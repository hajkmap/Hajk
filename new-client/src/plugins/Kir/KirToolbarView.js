...
import { IconPolygon, IconRect, IconLine, IconPoint } from "../Fir/FirIcons";
...
class KirToolbarView extends React.PureComponent {
  state = {
    tools: {
  ...
  initListeners = () => {
    this.localObserver.subscribe("kir.search.clear", () => {
      this.deactivateDraw();
  ...
  handleDeleteClick = (e) => {
    ...
    this.model.map.forEachFeatureAtPixel(e.pixel, (feature, layer) => {
      ...
      const kirType = feature.get("kir_type");
      if (kirType && (kirType === "draw" || kirType === "buffer") && first) {
        let findFn = null;
        ...
        } else if (kirType === "buffer") {
          findFn = (f) => {
            return feature.get("owner_ol_uid") === f.ol_uid;
          };
        }

        this.model.layers[kirType].getSource().removeFeature(feature);
        const layerName = kirType === "draw" ? "buffer" : "draw";
  ...
  deactivateDraw = () => {
    ...
    if (this.interaction) {
      ...
      this.model.map.clickLock.delete("kir-draw");
  ...
  activateDraw = () => {
    ...
    this.model.map.clickLock.add("kir-draw");
  ...
  handleFileSelection = (e) => {
    if (e && e.target) {
      ...
      if (e.target.files.length > 0) {
        this.localObserver.publish("kir.file.import", e.target.files[0]);
        setTimeout(() => {
          e.target.value = "";
        }, 500);
      }
          ...
          <div className={classes.containerTopDoublePadded}>
            <TextField
              ...
              onChange={(e) => {
                ...
                this.localObserver.publish("kir.layers.bufferValueChanged", {
                  value: bufferValue,
                });
              }}
              onFocus={(e) => {
...
export default withStyles(styles)(withSnackbar(KirToolbarView));
